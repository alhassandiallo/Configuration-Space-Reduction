...
def integrated_gradients(inputs, model, target_label_idx, calculate_outputs_and_gradients, steps, baseline):
    if baseline is None:
        baseline = 0 * inputs 
...
def random_baseline_integrated_gradients(inputs, model, target_label_idx, calculate_outputs_and_gradients, steps, num_random_trials):
    ...
    for i in range(num_random_trials):
        predictions, integrated_grad = integrated_gradients(inputs, model, target_label_idx, calculate_outputs_and_gradients, \
                                                baseline=None, steps=steps)
        all_intgrads.append(integrated_grad)
        #print('the trial number is: {}'.format(i))
    avg_intgrads = np.average(np.array(all_intgrads), axis=0)
    return predictions, avg_intgrads
