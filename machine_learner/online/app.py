...
def training_testing():
    try:
        if request.method == 'POST':
            ...
            if mode == 'training':
                if version == 'v1':
                    return jsonify(classification.v1_training(dataset))
                elif version == 'v2':
                    return jsonify(classification.v2_training(dataset))

            elif mode == 'testing':
                if version == 'v1':
                    return jsonify(attributions(dataset))
                elif version == 'v2':
                    return jsonify(classification.v2_testing(dataset))
